...
const s: Record<string, React.CSSProperties> = {
  ...
  title: { fontSize: 20, fontWeight: 700, color: '#1a3a5c', display: 'flex', alignItems: 'center', gap: 10 },
  subtitle: { fontSize: 13, color: '#64748b', marginTop: 4 },
  headerActions: { display: 'flex', gap: 10 },
  // 大按钮样式 (G004规范：44px+高度)
  btnLarge: {
    ...
    boxShadow: '0 2px 6px rgba(26,58,92,0.25)', minHeight: 44,
  },
  btnLargeSuccess: {
    display: 'inline-flex', alignItems: 'center', gap: 8,
    background: '#16a34a', color: '#fff', border: 'none', borderRadius: 8,
    padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer',
    boxShadow: '0 2px 6px rgba(22,163,74,0.25)', minHeight: 44,
  },
  btnLargeWarning: {
    display: 'inline-flex', alignItems: 'center', gap: 8,
    background: '#d97706', color: '#fff', border: 'none', borderRadius: 8,
    padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer',
    boxShadow: '0 2px 6px rgba(217,119,6,0.25)', minHeight: 44,
  },
  btnLargeDanger: {
    display: 'inline-flex', alignItems: 'center', gap: 8,
    background: '#dc2626', color: '#fff', border: 'none', borderRadius: 8,
    padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer',
    boxShadow: '0 2px 6px rgba(220,38,38,0.25)', minHeight: 44,
  },
  btnLargeInfo: {
    display: 'inline-flex', alignItems: 'center', gap: 8,
    background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8,
    padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer',
    boxShadow: '0 2px 6px rgba(37,99,235,0.25)', minHeight: 44,
  },
  // 小按钮
  btnIcon: {
  ...
  panelTitle: { fontSize: 14, fontWeight: 700, color: '#1a3a5c', display: 'flex', alignItems: 'center', gap: 6 },
  ...
  kpiCardPrimary: {
    background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', borderRadius: 12, padding: 20,
    border: 'none',
  },
  kpiLabel: { fontSize: 13, color: '#64748b', margin: 0 },
  kpiLabelLight: { fontSize: 13, color: 'rgba(255,255,255,0.8)', margin: 0 },
  kpiValue: { fontSize: 28, fontWeight: 700, color: '#1f2937', margin: '8px 0 0 0' },
  kpiValueLight: { fontSize: 28, fontWeight: 700, color: '#fff', margin: '8px 0 0 0' },
  kpiSub: { fontSize: 12, color: '#64748b', margin: '4px 0 0 0', display: 'flex', alignItems: 'center', gap: 4 },
  kpiSubLight: { fontSize: 12, color: 'rgba(255,255,255,0.8)', margin: '4px 0 0 0', display: 'flex', alignItems: 'center', gap: 4 },
  // 表格
  ...
  cadeTitle: { fontSize: 14, fontWeight: 600, color: '#fff', display: 'flex', alignItems: 'center', gap: 8 },
  ...
  cadeStat: {
    background: 'rgba(255,255,255,0.1)', borderRadius: 6, padding: '8px 12px', textAlign: 'center' as const,
  },
  ...
  modelIcon: {
    width: 48, height: 48, borderRadius: 10, background: 'linear-gradient(135deg, #667eea, #764ba2)',
  ...
  realtimeItemActive: {
    padding: '12px 14px', border: '2px solid #2563eb', borderRadius: 8,
    marginBottom: 8, cursor: 'pointer', background: '#f0f7ff', transition: 'all 0.2s',
  },
  // 状态指示器
  statusDot: {
    width: 8, height: 8, borderRadius: '50%', display: 'inline-block',
  },
  statusOnline: { background: '#22c55e', boxShadow: '0 0 6px #22c55e' },
  statusOffline: { background: '#ef4444' },
  statusWarning: { background: '#f59e0b' },
  // 空状态
...
// Types
// ============================================================
type TabKey = 'realtime' | 'history' | 'scoring' | 'models'
...
interface Detection {
  id: string
  frame: number
  type: 'polyp' | 'lesion' | 'bleeding' | 'other'
  confidence: number
  x: number
  y: number
  width: number
...
interface AIModel {
  ...
  name: string
  version: string
  type: 'cade' | 'jmanbi' | 'bbps' | 'report' | 'other',
  description: string
...
const generateScoringRecords = (): QCScoring[] => {
  const patients = ['张伟', '王芳', '李明', '刘洋', '陈静', '杨帆', '赵雷', '周婷']
  const types = ['胃镜', '肠镜']
...
export default function AIQCPage() {
  ...
  const filteredHistory = historyRecords.filter(r => {
    const matchSearch = r.patientName.includes(searchKeyword) ||
                        r.examType.includes(searchKeyword) ||
    ...
    <div style={s.pageWrapper}>
      ...
      <div style={s.pageHeader}>
        <div>
          ...
          <p style={s.subtitle}>智能辅助质控 · CADe实时检测 · AI模型管理</p>
        </div>
        <div style={s.headerActions}>
          <button style={s.btnLargeInfo}>
            <Upload size={16} />导入数据
          </button>
          ...
          <div key={kpi.label} style={s.kpiCard}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                ...
                <BrainCircuit size={24} color={kpi.color} />
              </div>
              <div>
      ...
      <div style={s.tabNav}>
        <button
          style={activeTab === 'realtime' ? s.tabBtnActive : s.tabBtn}
          onClick={() => setActiveTab('realtime')}
        >
          <Zap size={16} />实时检测
        </button>
        <button
          style={activeTab === 'history' ? s.tabBtnActive : s.tabBtn}
          onClick={() => setActiveTab('history')}
        >
          <FileSearch size={16} />历史记录
        </button>
        <button
          style={activeTab === 'scoring' ? s.tabBtnActive : s.tabBtn}
          onClick={() => setActiveTab('scoring')}
        >
          <Award size={16} />质控评分
        </button>
        <button
          style={activeTab === 'models' ? s.tabBtnActive : s.tabBtn}
          ...
          <div style={s.cadePanel}>
            <div style={s.cadeHeader}>
              ...
              <div style={s.cadeStatus}>
                <span style={{ ...s.statusDot, ...s.statusOnline }} />
                检测中
              </div>
            </div>
            ...
            <div style={s.cadeControls}>
              <button style={{ ...s.btnLarge, padding: '8px 16px', minHeight: 40 }}>
                <Play size={14} />开始检测
              </button>
              <button style={{ ...s.btnLargeWarning, padding: '8px 16px', minHeight: 40 }}>
                <Pause size={14} />暂停
              </button>
              <button style={{ ...s.btnLargeDanger, padding: '8px 16px', minHeight: 40 }}>
                <RefreshCw size={14} />重置
              </button>
            </div>
            <div style={s.cadeStats}>
              <div style={s.cadeStat}>
                <div style={s.cadeStatLabel}>检测帧数</div>
                <div style={s.cadeStatValue}>1,256</div>
              </div>
              <div style={s.cadeStat}>
                <div style={s.cadeStatLabel}>发现可疑</div>
                <div style={{ ...s.cadeStatValue, color: '#ef4444' }}>3</div>
              </div>
              <div style={s.cadeStat}>
                <div style={s.cadeStatLabel}>检测时长</div>
                <div style={s.cadeStatValue}>00:42</div>
              </div>
              <div style={s.cadeStat}>
                <div style={s.cadeStatLabel}>置信度</div>
                <div style={{ ...s.cadeStatValue, color: '#22c55e' }}>94.5%</div>
              </div>
            </div>
          </div>
          ...
          <div style={s.panel}>
            ...
            <div style={s.panelBody}>
              {[
                ...
                <div key={idx} style={idx === 0 ? s.realtimeItemActive : s.realtimeItem}>
                  ...
                  <tbody>
                    {paginatedHistory.map((record) => (
                      <tr key={record.id}>
                        ...
                        <td style={s.td}>
                          <span style={{ ...s.badge, ...s.badgeInfo }}>{record.examType}</span>
                        </td>
                        <td style={s.td}>
                          <span style={{
                            fontWeight: 600,
                            color: record.score >= 90 ? '#16a34a' : record.score >= 80 ? '#d97706' : '#dc2626'
                          }}>
                            {record.score}分
                          </span>
                        </td>
                        <td style={s.td}>
                          <span style={{
                            ...s.badge,
                            ...(record.grade === '优秀' ? s.badgeSuccess :
                               record.grade === '良好' ? s.badgeInfo :
                               record.grade === '合格' ? s.badgeWarning : s.badgeDanger)
                          }}>
                            {record.grade}
                          </span>
                        </td>
                        ...
                        <td style={s.td}>
                          <div style={{ display: 'flex', gap: 4 }}>
                            <button style={s.btnIcon} title="查看详情">
                              <Eye size={14} />
                            </button>
                            <button style={s.btnIcon} title="导出报告">
                              <Download size={14} />
                            </button>
                          </div>
                        </td>
            ...
            {filteredHistory.length > 0 && (
              <div style={s.pagination}>
                <div style={s.pageInfo}>
                  显示 {(page - 1) * pageSize + 1}-{Math.min(page * pageSize, filteredHistory.length)} 条，共 {filteredHistory.length} 条
                </div>
                <div style={s.pageBtns}>
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button
                        key={p}
                        style={page === p ? s.pageBtnActive : s.pageBtn}
                        onClick={() => setPage(p)}
                      >
                        {p}
                      </button>
                    ...
                    {filteredScoring.map((record) => (
                      <tr key={record.id}>
                        ...
                        <td style={s.td}>
                          <span style={{
                            ...s.badge,
                            ...(record.totalScore >= 90 ? s.badgeSuccess :
                               record.totalScore >= 80 ? s.badgeWarning : s.badgeDanger)
                          }}>
                            {record.totalScore}分
                          </span>
                        </td>
                        <td style={s.td}>
                          {record.deductions.length > 0 ? (
                            <span style={{ fontSize: 11, color: '#dc2626' }}>
                              {record.deductions.map(d => d.item).join('、')}
                            </span>
                          ) : (
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={s.btnLargeSuccess}>
                <Plus size={16} />新增模型
              </button>
              <button style={s.btnLarge}>
                <RefreshCw size={16} />同步模型
              </button>
            </div>
            <button style={s.btnLargeInfo}>
              ...
              <div key={model.id} style={s.modelCard}>
                <div style={s.modelCardHeader}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                    <span style={{
                      ...s.badge,
                      ...(model.status === 'active' ? s.badgeSuccess :
                         model.status === 'training' ? s.badgeWarning : s.badgeDanger)
                    }}>
                      {model.status === 'active' ? '运行中' :
                       model.status === 'training' ? '训练中' : '已停用'}
                    </span>
                  </div>
                </div>
                ...
                <div style={s.modelStats}>
                  <div style={s.modelStat}>
                    <div style={s.modelStatValue}>{model.accuracy}%</div>
                    <div style={s.modelStatLabel}>准确率</div>
                  </div>
                  <div style={s.modelStat}>
                    <div style={s.modelStatValue}>{model.precision}%</div>
                    <div style={s.modelStatLabel}>精确率</div>
                  </div>
                  <div style={s.modelStat}>
                    <div style={s.modelStatValue}>{model.recall}%</div>
                    <div style={s.modelStatLabel}>召回率</div>
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>
                    已检测 {model.detections.toLocaleString()} 次
                  </span>
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>
                    更新于 {model.lastUpdated}
                  </span>
                </div>
                <div style={s.modelActions}>
                  {model.status === 'active' ? (
                    <>
                      <button style={s.btnWarning} title="暂停模型">
                        <Pause size={12} />暂停
                      </button>
                      <button style={s.btnDanger} title="停用模型">
                        <X size={12} />停用
                      </button>
                    </>
                  ) : model.status === 'training' ? (
                    <button style={s.btnSuccess} title="查看训练进度">
                      <TrendingUp size={12} />训练中
                    </button>
                  ) : (
                    <button style={s.btnSuccess} title="启用模型">
                      <Check size={12} />启用
                    </button>
                  )}
                  <button style={s.btnIcon} title="模型配置">
                    <Settings size={12} />
                  </button>
                </div>
              </div>

...
export default function ReportWritePage() {
  ...
  const [templates] = useState<ReportTemplate[]>(initialReportTemplates)
  ...
  // 关联的检查信息
  const [linkedExam, setLinkedExam] = useState<typeof initialEndoscopyExams[0] | null>(null)
  ...
  // 活检管理
  const [biopsySites, setBiopsySites] = useState<BiopsySite[]>([])
  ...
  // AI 辅助
  const [aiContent, setAiContent] = useState<{ findings: string; conclusion: string } | null>(null)
  const [showAIBox, setShowAIBox] = useState(false)
  const [aiLoading, setAiLoading] = useState(false)

  // 术语提示
  const [termHints, setTermHints] = useState<{ keyword: string; suggestions: string[] } | null>(null)
  ...
  // ========== 新增：质量评分 ==========
  const [qualityScore, setQualityScore] = useState<{ total: number; breakdown: { key: string; label: string; score: number; detail: string; weight: number }[] } | null>(null)

  // ========== 新增：历史参考 ==========
  const [historyReports, setHistoryReports] = useState<{ id: string; date: string; examType: string; findings: string; conclusion: string }[]>([])
  ...
  // ========== 新增：图片标注框拖拽 ==========
  const [annDragStart, setAnnDragStart] = useState<{ x: number; y: number } | null>(null)
  ...
  const handleSaveDraft = () => {
    ...
    if (modalMode === 'new') {
      ...
      setReports(prev => [{ ...editingReport, id, status: '书写中', imageUrls: finalImages, createdTime: now, updatedTime: now }, ...prev])
    } else {
      setReports(prev => prev.map(r => r.id === editingReport.id ? { ...editingReport, imageUrls: finalImages, status: r.status === '未开始' ? '书写中' : r.status, updatedTime: now } as EndoscopyReport : r))
    }
    setFormErrors([])
  ...
  const handleSubmitReport = () => {
    ...
    if (modalMode === 'new') {
      const id = 'RPT' + Date.now().toString().slice(-6)
      setReports(prev => [{ ...editingReport, id, status: '待审核', imageUrls: finalImages, createdTime: now, updatedTime: now }, ...prev])
    } else {
      setReports(prev => prev.map(r => r.id === editingReport.id ? { ...editingReport, imageUrls: finalImages, status: '待审核', updatedTime: now } as EndoscopyReport : r))
    }
  ...
  // ========== 新增：图片标注交互 ==========
  const [annotatingImg, setAnnotatingImg] = useState<ReportImage | null>(null)
  const [annBox, setAnnBox] = useState({ x: 20, y: 20, w: 80, h: 60 })
  const [annDrawing, setAnnDrawing] = useState(false)
  const [annStart, setAnnStart] = useState<{ x: number; y: number } | null>(null)
